...
export class BlogCommentService {
  ...
  create = (model: BlogCommentCreate):Observable<BlogComment>  => 
   this.http.post<BlogComment>(`${environment.webApi}/BlogComment`,model)
  

  delete = (blogCommentId: number): Observable<number> =>  
   this.http.delete<number>(`${environment.webApi}/BlogComment/${blogCommentId}`)


  getAll = (blogId: number): Observable<BlogComment[]> => 
    this.http.get<BlogComment[]>(`${environment.webApi}/BlogComment/${blogId}`)
